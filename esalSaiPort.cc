...
static bool restorePorts(SaiPortEntry* portTable, int portTableSize) {
    ...
    for (int i = 0; i < portTableSize; i++) {
        ...
        bool ls;
        status &= VendorGetPortLinkState(lPort, &ls);
        if (!status) {
...
bool portWarmBootSaveHandler() {
    ...
    for (int i = 0; i < portTableSize; i++) {
        ...
        status &= saiUtils.GetLogicalPort(0, pPort, &lPort);
        if (!status) {
            std::cout << "Error GetLogicalPort: " << pPort << std::endl;
            continue;
        }
    ...
    status &= serializePortTableConfig(portTable, portTableSize, BACKUP_FILE_PORT);
    if (!status) {
