...
def WrappedVendorRcToString (rc, strErr):
...
def WrappedVendorCreateVlan (vlanid):
...
def WrappedVendorDeleteVlan (vlanid):
    w_vlanid = ctypes.c_uint16(vlanid)
...
def WrappedVendorAddPortsToVlan (vlanid, numPorts, ports):
    w_vlanid = ctypes.c_uint16(vlanid)
...
def WrappedVendorDeletePortsFromVlan (vlanid, numPorts, ports):
    w_vlanid = ctypes.c_uint16(vlanid)
    w_numPorts = ctypes.c_uint16(numPorts)
    w_ports = (ctypes.c_uint16*len(ports))(*ports)
...
def WrappedVendorGetPortsInVlan (vlanId, numPorts, ports):
    w_vlanId = ctypes.c_uint16(vlanId)
    w_numPorts = ctypes.c_uint16(numPorts)
    w_ports = (ctypes.c_uint16*len(ports))(*ports)
...
def WrappedVendorSetPortDefaultVlan (port, vlanId):
...
def WrappedVendorGetPortDefaultVlan (port, vlanId):
    w_port = ctypes.c_uint16(port)
    w_vlanId = ctypes.c_uint16(vlanId)
...
def WrappedVendorDeletePortDefaultVlan (port, vlanId):
    w_port = ctypes.c_uint16(port)
    w_vlanId = ctypes.c_uint16(vlanId)
...
def WrappedVendorSetPortEgress (port, numPorts, ports):
    w_port = ctypes.c_uint16(port)
    w_numPorts = ctypes.c_uint16(numPorts)
    w_ports = (ctypes.c_uint16*len(ports))(*ports)
...
def WrappedVendorGetPortAutoNeg (port, aneg):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorGetPortLinkState (port, ls):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorMirrorPort (srcPort, dstPort):
...
def WrappedVendorRemoveMirrorPort (srcPort, dstPort):
    w_srcPort = ctypes.c_uint16(srcPort)
    w_dstPort = ctypes.c_uint16(dstPort)
...
def WrappedVendorEnablePort (port):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorDisablePort (port):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorTagPacketsOnIngress (port):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorStripTagsOnEgress (port):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorDropTaggedPacketsOnIngress (port):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorDropUntaggedPacketsOnIngress (port):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorAddPacketFilter (filter, length):
...
def WrappedVendorDeletePacketFilter (filterName):
...
def WrappedVendorSendPacket (port, length, packet):
    w_port = ctypes.c_uint16(port)
    w_length = ctypes.c_uint16(length)
...
def WrappedVendorPurgeMacEntriesPerPort (port):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorPurgeMacEntries ():
...
def WrappedVendorGetMacTbl (port, numMacs, macs):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorDisableMacLearningPerVlan (vlan):
...
def WrappedVendorEnableMacLearningPerVlan (vlan):
    w_vlan = ctypes.c_uint16(vlan)
...
def WrappedVendorDisableMacLearningPerPort (port):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorEnableMacLearningPerPort (port):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorSetFrameMax (port, size):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorGetFrameMax (port, size):
    w_port = ctypes.c_uint16(port)
    w_size = ctypes.c_uint16(size)
...
def WrappedVendorSetPortAdvertAbility (port, cap):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorGetPortAdvertAbility (port, advert):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorWarmRestartRequest ():
...
def WrappedVendorResetPort (port):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorGetL2Pm (used_len, max_len, msg):
...
def WrappedVendorReadReg (port, reg, val):
    w_port = ctypes.c_uint16(port)
...
def WrappedVendorWriteReg (port, reg, val):
    w_port = ctypes.c_uint16(port)
    w_reg = ctypes.c_uint16(reg)
    w_val = ctypes.c_uint16(val)
